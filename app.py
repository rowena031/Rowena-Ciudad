...
# Replace 'YOUR_PASSWORD' with your actual hosting password
db_config = {
    'host': 'sql311.byetcluster.com',
    'user': 'b8_41119919',
    'password': '010311', 
    'database': 'b8_41119919_student'
}

def get_db_connection():
    return mysql.connector.connect(**db_config)

# Removed init_db() because your phpMyAdmin SQL already created the table

login_page = """
<html>
<head><title>Login</title>
<style>
body{font-family:Arial;background:linear-gradient(135deg,#9CAF88,#7A9D76);display:flex;justify-content:center;align-items:center;height:100vh;color:white;}
.box{background:rgba(255,255,255,0.2);padding:40px;border-radius:10px;}
input{padding:10px;margin:10px;border:none;border-radius:5px;}
button{padding:10px 20px;border:none;background:#6B8F71;color:white;border-radius:5px;cursor:pointer;}
</style>
</head>
<body>
<div class="box">
...
<style>
body{font-family:Arial;background:linear-gradient(135deg,#9CAF88,#7A9D76);color:white;text-align:center;padding:30px;}
.container{background:rgba(255,255,255,0.2);padding:25px;border-radius:15px;width:750px;margin:auto;}
...
table{width:100%;margin-top:20px;border-collapse:collapse;background:rgba(255,255,255,0.1);}
...
<input id="id" placeholder="ID (Auto)" readonly>
...
function drawTable(data){
    ...
    table.innerHTML=`<tr><th>ID</th><th>Name</th><th>Grade</th><th>Section</th><th>Action</th></tr>`;
    data.forEach(s=>{
        table.innerHTML+=`<tr>
            <td>${s.id}</td><td>${s.name}</td><td>${s.grade}</td><td>${s.section}</td>
            <td>
...
function drawChart(names,grades){
    ...
    chart=new Chart(document.getElementById("chart"),{
        ...
        data:{
            labels:names,
            datasets:[{label:"Grade Level",data:grades,backgroundColor:"#6B8F71"}]
        }
    });
}
function addStudent(){
...
function deleteStudent(id){
    fetch("/students/"+id,{method:"DELETE"}).then(()=>loadStudents());
}
loadStudents();
</script>
</body>
